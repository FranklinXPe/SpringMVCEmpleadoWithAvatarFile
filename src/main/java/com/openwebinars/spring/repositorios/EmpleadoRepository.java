...
public  interface EmpleadoRepository extends JpaRepository<Empleado, Long>{
	// Que busque los empleados con el nombre, email y telefono que contengan estos valores
	...
	@Query(value="SELECT * FROM EMPLEADO WHERE LOWER(NOMBRE) LIKE CONCAT('%',?1,'%') OR LOWER(EMAIL) LIKE CONCAT('%',?1,'%') OR LOWER(TELEFONO) LIKE CONCAT('%',?1,'%')",
			nativeQuery = true)
